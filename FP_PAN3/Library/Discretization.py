...
class Turn:
    def __init__(self, drone, initPoint, turnPoint, endPoint, turnCoefficient):
        ...
        if self.R >= MIN_RADIUS:
            ...
            self.deltaBearing, self.deltaAttack =IdentifyTurn(initPoint, turnPoint, endPoint, margin = 0, divide = True)
            maxHSpeed, maxVSpeed = self.drone.GetMaxSpeed(radius, self.deltaBearing[1], self.deltaAttack[1])
            for i in range(0, ACCURACY+1):
                ...
                self.speed = min([divide(maxHSpeed, np.cos(s_attack*np.pi/180)), divide(maxVSpeed, np.abs(np.sin(s_attack*np.pi/180)))]+ [np.sqrt(divide(self.R*self.drone.horizontalAccel,np.cos(a_attack*np.pi/180))),np.sqrt(divide(self.R*self.drone.verticalAccel,np.abs(np.sin(a_attack*np.pi/180))))])
            if max([self.initPoint.FPLvel, self.turnPoint.FPLvel]) < self.speed:
                self.speed = max([self.initPoint.FPLvel, self.turnPoint.FPLvel])
...
class Straight: 
    ...
    def SetEndSpeed(self, endSpeed):  #1. initSpeed || endSpeed ARE NOT ASSUMABLE, if initSpeed > endSpeed --> initSpeed changes returns false, else endSpeed changes returns true
                                      #2. initSpeed  || endSpeed ARE ASSUMABLE --> returns true
        self.distance = [0]
        self.time = [0]
        ...
        #No distance (initPoint == endPoint)
        if self.totalDistance == 0:
            self.acceleration =  [0]
        ...
        #1. Not enought distance to reach the end speed
        ...
        #2. Enough distance to reach the end speed
        else:
            self.acceleration =  [self.maxAccel]
            ...
            #Enought distance to reach the Cruising/Max speed
            if (maxSpeed**2-max([self.initSpeed ,endSpeed])**2)/(2*self.maxAccel) < (self.totalDistance-np.abs(endSpeed**2-self.initSpeed**2)/(2*self.maxAccel))/2:
                ...
                self.distance.append(self.totalDistance-(self.endSpeed**2-maxSpeed**2)/(2*sign*self.maxAccel))
                self.time.append(self.time[-1]+(self.distance[-1]-self.distance[-2])/maxSpeed)
                self.speed.append(maxSpeed)
                self.acceleration.append(sign*self.maxAccel)
            #Not enought distance to reach the Cruising/Max speed
            else:
    ...
    def GetPoint(self, *, distance = None, time = None):
        ...
        if distance!= None:
            if self.acceleration[i] == 0:
                ...
                time = self.time[i]+(-self.speed[i]+np.sqrt(self.speed[i]**2+2*self.acceleration[i]*(distance-self.distance[i])))/(self.acceleration[i])
        elif time != None:
