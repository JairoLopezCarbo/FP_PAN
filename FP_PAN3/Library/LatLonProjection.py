...
class Point:
    ...
    def Copy(self):
        newPoint = Point(self.lat, self.lon, self.alt, time = self.time)
        newPoint.HSpeed, newPoint.VSpeed=self.HSpeed, self.VSpeed 
        newPoint.HAccel, newPoint.VAccel=self.HAccel, self.VAccel 
        newPoint.bearing=self.bearing
        newPoint.maxRadius=self.maxRadius 
        newPoint.FPLvel = self.FPLvel
        return newPoint
...
def LatLonDistance(pt1, pt2): #meters[m]
    if pt1.lat == pt2.lat and pt1.lon == pt2.lon:
        return 0
